...
// Return the probability of image being of class
Clarifai.prototype._getProbsURL  = function( url, select_classes, localId, resultHandler, retry ) {
	if( this._throttled ) 
		// the host has throttled us, so there's no point in sending the request
		// just immediately return the throttled status
		resultHandler( { 'status_code': 'ERROR_THROTTLED',
	...
	var form = new Array();
	form["url"] = url;
	form["select_classes"] = select_classes;
...
// Return the probability of image being of class
Clarifai.prototype._getProbsEncodedData  = function( encodeddata, select_classes, localId, resultHandler, retry ) {
	if( this._throttled ) 
		// the host has throttled us, so there's no point in sending the request
		// just immediately return the throttled status
		resultHandler( { 'status_code': 'ERROR_THROTTLED',
...
Clarifai.prototype.getProbsURL = function( url, select_class, localId, callback ) {

  retry = function() {
      this.getProbsURL( url, select_class, localId, callback );
  }.bind(this);

  this._getProbsURL( url, select_class, localId,
    callback,	
    retry );

}

Clarifai.prototype.getProbsEncodedData = function( encodedData, select_class, localId, callback ) {

  retry = function() {
      this.getProbsEncodedData( encodedData, select_class, localId, callback );
  }.bind(this);

  this._getProbsEncodedData( encodedData, select_class, localId,
    callback,	
    retry );
